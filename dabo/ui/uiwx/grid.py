...
if dabo.ui.phoenix:
	wxGTclass = wx.grid.GridTableBase
else:
	wxGTclass = wx.grid.PyGridTableBase

class dGridDataTable(wxGTclass):
	...
	def convertType(self, typ):
		...
		if lowtyp in (sixInt, long, "int", "integer", "bigint", "i", "long"):
	...
	def _setEditor(self, row):
		...
		if edClass:
			...
			if kwargs:
				...
				# Phoenix doesn't support kwargs if the dict is empty
				editor = edClass()
...
#				editor.SetPrecision(self.Precision)
	...
	def _setAutoHorizontalAlignment(self):
		dt = self.DataType
		if isinstance(dt, sixBasestring):
...
#		self.bindEvent(dEvents.GridContextMenu, self._onContextMenu)
	...
	def buildFromDataSet(self, ds, keyCaption=None,
		...
		if isinstance(ds, sixBasestring) or isinstance(ds, dabo.biz.dBizobj):
	...
	def autoSizeCol(self, colNum, persist=False):
		...
		"""
		if isinstance(colNum, sixBasestring) and colNum.lower() == "all":
			self.BeginBatch()
	...
	def _paintHeader(self, updateBox=None, paintevent=False):
		...
		for idx, col in enumerate(self._columns):
			...
			if colObj.DataField == self.sortedColumn:
				...
				if isinstance(brushColor, sixBasestring):
			...
			if self.customSort:
				...
				if dataType is None:
					...
					if isinstance(f, datetime.date):
						...
						dataType = None
					sortingStrings = isinstance(sortList[0][0], sixBasestring)
				else:
	...
	def runIncSearch(self):
		...
		for row in sortList:
			if row[0] is not None:
				compString = isinstance(row[0], sixBasestring)
				break
		...
		if compString:
			if caseSensitive:
				mtchs = [vv for vv in sortList
						if isinstance(vv[0], sixBasestring) and vv[0].startswith(srchVal)]
			else:
				srchVal = srchVal.lower()
				mtchs = [vv for vv in sortList
						if isinstance(vv[0], sixBasestring) and vv[0].lower().startswith(srchVal)]
		else:
			mtchs = [vv for vv in sortList
					if vv[0] == srchVal]
		if mtchs:
			...
			for fldval, row in sortList:
				if not compString or caseSensitive:
					...
					match = (isinstance(fldval, sixBasestring) and fldval.lower() == srchVal)
				if match:
					newRow = row
					break
				else:
					...
					if compString and not caseSensitive and isinstance(fldval, sixBasestring):
		...
		if action == "Find":
			try:
				while True:
					...
					if isinstance(targetVal, (sixBasestring, datetime.datetime, datetime.date)):
	...
	def _resolveColumn(self, colOrIdx, returnColumn=True, logOnly=False):
		...
		"""
		if isinstance(colOrIdx, (sixInt, sixLong)):
	...
	def _getTable(self):
		...
		if not tbl:
			try:
				...
				# TODO: Phoenix, this can not be False for the moment
